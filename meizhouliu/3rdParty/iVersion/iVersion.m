...
{
    return _inThisVersionTitle ?: [self localizedStringForKey:iVersionInThisVersionTitleKey withDefault:@"检测到新本"];
}

- (NSString *)updateAvailableTitle
{
    return _updateAvailableTitle ?: [self localizedStringForKey:iVersionUpdateAvailableTitleKey withDefault:@"检测到新本"];
}
...
{
    return _downloadButtonLabel ?: [self localizedStringForKey:iVersionDownloadButtonKey withDefault:@"马上更新"];
}

- (NSString *)remindButtonLabel
{
    return _remindButtonLabel ?: [self localizedStringForKey:iVersionRemindButtonKey withDefault:@"稍等一会"];
}
...
{
    if (appStoreID == 0) {
        [[NSUserDefaults standardUserDefaults] removeObjectForKey:iVersionAppStoreIDKey];
    } else {
...
- (NSString *)versionDetails:(NSString *)version inDict:(NSDictionary *)dict
{
    ...
    if ([versionData isKindOfClass:[NSArray class]])
    {
        NSArray *versionInfos = (NSArray *)versionData;
        if (versionInfos.count < 6) { // info not complete
            return nil;
        }
        //set app update address
        NSString *appIdFromService = versionInfos[1];
        if (! [appIdFromService isEqualToString:@"0"]) {
            NSString *updateURL = [NSString stringWithFormat:iVersioniOSAppStoreURLFormat, versionInfos[1]];
            self.updateURL = [NSURL URLWithString:updateURL];
        }
        // version detail
        NSString *versionUpdateDetails = versionInfos[5];
        if (! isEmptyString(versionUpdateDetails)) {
            [iVersion sharedInstance].updateAvailableDetail = versionUpdateDetails;
        }
        //force update flag
        if ([[versionData objectAtIndex:3] isEqualToString:@"true"]) {
            return @"YES";
        }
        return [versionData objectAtIndex:5];
    }
            ...
            cancelButton:(NSString *)cancelButton
{
    ...
    return [[UIAlertView alloc] initWithTitle:title
                            ...
                            cancelButtonTitle:defaultButton
                            otherButtonTitles:cancelButton, nil, nil];
    ...
    if (self.checkingForNewVersion)
    {
        ...
        if (details)
        {
            ...
            if (showDetails && !self.visibleRemoteAlert)
            {
                ...
                NSString *detailsMsg = self.updateAvailableDetail ? self.updateAvailableDetail : details;
