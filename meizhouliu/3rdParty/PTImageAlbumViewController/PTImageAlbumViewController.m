...
- (void)viewDidLoad
{
    ...
    self.photoAlbumView.loadingImage = [UIImage imageNamed:@"Default_Loc_Big"];
...
- (UIImage *)loadThumbnailImageAtIndex:(NSInteger)index
{
    ...
    UIImage *image = [self.thumbnailImageCache objectWithName:[NSString stringWithFormat:@"%@", @(index)]];
    if (image == nil) {
          ...
          originalPhotoDimensions:(CGSize *)originalPhotoDimensions
{
    ...
    *originalPhotoDimensions = [self.imageAlbumView sizeForImageAtIndex:photoIndex];
    ...
    UIImage *image = [self.highQualityImageCache objectWithName:[NSString stringWithFormat:@"%@", @(photoIndex)]];
    if (image) {
        *photoSize = NIPhotoScrollViewPhotoSizeOriginal;
        ...
        // only we have a source for high quality image
        if (source) {
            // TODO remove duplicate
            // >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
            NSURL *url = nil;
